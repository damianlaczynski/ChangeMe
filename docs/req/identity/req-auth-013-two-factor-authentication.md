...
- After successful setup, the application opens **Issues list**, subject to **Combined account compliance gates** (password expiration is resolved before two-factor setup when both apply on the same sign-in). See `docs/req/_shared/compliance-gates.md`.
...
See `docs/req/_shared/compliance-gates.md` for the full ordered gate list.
...
- Passkeys (WebAuthn): `docs/req/passkeys/` (REQ-PKY-001 through REQ-PKY-007). **Passkey satisfies two-factor** deployment setting interacts with this REQ; see REQ-PKY-001 and REQ-PKY-006.
