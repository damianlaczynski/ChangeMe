...
- `docker-compose.yml` starts the local stack: Angular frontend, ASP.NET backend, PostgreSQL, and MailHog. The same file defines `backend-tests` (Compose profile `test`): an SDK container that runs `dotnet test` on the backend solution with the repository mounted from the host. Optional scans live in `docker-compose.analyze.yml` — see [security-analysis.md](../technical/security-analysis.md).
