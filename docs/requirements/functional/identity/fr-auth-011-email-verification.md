...
- A user with **Deactivated** false and **Email verified** false cannot sign in when verification is enabled (FR-AUTH-001); **Login** shows form-level error **`Verify your email before signing in.`** with link **Resend verification email** → **Verify email**.
