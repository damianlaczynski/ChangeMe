...
- The frontend loads the list of enabled providers (key and display name only) from a public auth settings endpoint; secrets remain server-side. The same endpoint exposes **External providers enabled**, **External provider linking enabled**, and **Self-service email change enabled** (FR-AUTH-015).
- When **Trust identity provider MFA** is **true** (FR-AUTH-013), the backend evaluates the IdP **`amr`** claim (and provider-specific equivalents) on each external sign-in callback.
...
- For **new account creation** via OIDC on **Register** or **Login**, and for **Accept invitation** external onboarding (FR-AUTH-010), the provider email is used only when the identity provider asserts it is verified, when **Trust IdP email without email verified** is **enabled** for that provider, or when the token includes provider-specific verified email claims (for example **verified primary email**). If the email is missing or not treated as verified, the system does not create an account by email; an existing **External login** for this provider subject may still sign the user in.
...
See FR-AUTH-013 for deployment flag and password-sign-in rules. External sign-in behavior:
...
| Term               | Meaning                                                                                                                                                                                                                                                                              |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Profile email**  | The **current email** on the ChangeMe user account. Used for email/password sign-in, uniqueness (FR-USR-003), display on **My account**, and **every** application and auth notification (FR-AUTH-007, issue notifications).                                                         |
...
**Profile email** changes only through: registration, **Invite user**, first-time OIDC account creation (initial **Profile email** = verified provider email), **Change email** (FR-AUTH-015), or administrator **Edit user** (FR-USR-003).
...
The system maintains **at most one user account per normalized Profile email** (FR-USR-003).

Guest **Continue with {Display name}** on **Login** or **Register** does **not** link a provider to an existing account except **Accept invitation** external onboarding (FR-AUTH-010). To add Google or Microsoft to an existing account, the user signs in to ChangeMe first, then uses **Link {Display name}** on **My account**.
...
| User is **awaiting invitation acceptance**; verified **Provider email** matches invited **Profile email**         | Complete invitation and link provider per FR-AUTH-010 (only email-match link on guest OIDC).                                                                                                                                                                |
...
- External sign-in never bypasses app TOTP when **Trust identity provider MFA** is **disabled**, or when the IdP does not assert MFA. When **Trust identity provider MFA** is **enabled** and MFA is asserted, external sign-in may bypass **Two-factor verification** and **strict two-factor setup** per FR-AUTH-013.
...
| 2    | **Sensitive account actions** step-up (FR-AUTH-013) completes before OIDC starts.                                                                                                                                                                            |
...
| 7    | On success: message **`External sign-in method linked.`**; **External account linked** email to **Profile email** (FR-AUTH-007); refresh **My account**.                                                                                                     |
...
- **Out of scope:** guest **Link external account** screen; auto-linking a provider during **Login** or **Register** when **Provider email** matches an existing account (except **Accept invitation** per FR-AUTH-010).

### Login screen — external sign-in

- When **External providers enabled** is **true** and at least one provider is configured, **Login** shows **Continue with {Display name}** for each provider (FR-AUTH-001).
...
- **Link {Display name}** (when linking is enabled) and **Unlink** require **Sensitive account actions** step-up authentication (FR-AUTH-013) before the action completes.
...
- On successful link: message **`External sign-in method linked.`** and **External account linked** email (FR-AUTH-007).
- On successful unlink: message **`External sign-in method removed.`** and **External account unlinked** email (FR-AUTH-007).

### Set password (signed-in)

- Screen: **Set password**; linked from **My account** when the signed-in user is **external-only**.
- **Set password** requires **Sensitive account actions** step-up authentication (FR-AUTH-013) before submit.
- Same fields and validation as **Change password** except **Current password** is omitted (FR-AUTH-008).
- On success establishes a **local password** and sets **password last changed at**; does not revoke other sessions.
- Enables **Change password** (FR-AUTH-005) and **Forgot password** self-service thereafter.
...
- Password expiration (FR-AUTH-009) applies only when the user **has a local password**; **external-only** users are not redirected to **Required password change** for age reasons.
- **Change password** (FR-AUTH-005) requires a **local password**; **external-only** users use **Set password** first.

### Administrator unlink (external sign-in)

- Administrators may **Unlink** a provider from **User details** (FR-USR-004); requires **Users.Manage**.
...
- Sends **External account unlinked** email (FR-AUTH-007).

### User details (admin read-only)

- When **External providers enabled** is **true**, **User details** shows section **External sign-in methods**: linked **Provider** display names, **Linked at**, and per-row **Unlink** for administrators (FR-USR-004).

### Admin invite user and invitation

- **Invite user** (FR-INV-001): administrators onboard by **Profile email**; the invitee must use that address for **Accept invitation** external onboarding (FR-AUTH-010).

### Interaction with other auth flows

| Flow                                  | Behavior when external providers enabled                                                                                |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| **Login** (FR-AUTH-001)               | Email/password unchanged; provider buttons when configured.                                                             |
| **Register** (FR-AUTH-001)            | Provider buttons when public registration enabled.                                                                      |
| **Two-factor** (FR-AUTH-013)          | Applies after provider success unless **Trust identity provider MFA** and IdP MFA assertion apply.                      |
| **Email verification** (FR-AUTH-011)  | Provider-asserted verified email sets **Email verified** true on new account; existing unverified users remain blocked. |
| **Public registration** (FR-AUTH-012) | When disabled, provider sign-in for unknown emails fails with contact administrator message.                            |
| **Password expiration** (FR-AUTH-009) | Not evaluated for users **without a local password**.                                                                   |

### States and business rules

- **Link {Display name}** and **Unlink** require a signed-in session and **Sensitive account actions** step-up (FR-AUTH-013), except administrator **Unlink** on **User details**.
- After linking, all **External login** rows for the user share one **Profile email**, roles, sessions, and two-factor settings.
- Admin **Edit user** **Profile email** change (FR-USR-003): when **External providers enabled** is **true** and the user has at least one **External login**, **Edit user** shows notice **`External sign-in stays linked. Profile email is used for notifications; provider addresses may differ.`** Saving does **not** remove **External login** rows; administrators may **Unlink** providers from **User details** when appropriate.
- Self-service **Change email** (FR-AUTH-015): **External login** rows are retained; notice on **Change email** when the user has linked providers: **`External sign-in methods stay linked. Notifications stay on your profile email.`**
- **Out of scope:** SAML 2.0; social providers without email claim; automatic admin provisioning rules; SCIM; admin UI to configure providers at runtime; merging two user records with **different Profile emails** into one account; guest **Link external account** screen.
...
| ID             | Given                                                                                                            | When                                                                                                  | Then                                                                                                                                                                            |
| -------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-AUTH-014-01 | **External providers enabled** is **true**; at least one provider configured; guest on **Login**                 | User views sign-in options                                                                            | **Continue with {Display name}** button shown for each provider below email/password form, separated by **or** divider (FR-AUTH-001)                                            |
| AC-AUTH-014-02 | **External providers enabled** is **true**; guest on **Login**                                                   | User clicks **Continue with {Display name}**                                                          | OIDC authorization code flow with PKCE starts; user redirected to provider and returns to **External sign-in callback**                                                         |
| AC-AUTH-014-03 | **External providers enabled** is **false**; user is **external-only** (no local password)                       | User opens **Login**                                                                                  | Form-level message `External sign-in is unavailable. Contact an administrator or set a password when sign-in is available.`                                                     |
| AC-AUTH-014-04 | **External providers enabled** is **false**                                                                      | User views **Login**, **Register**, or **My account**                                                 | All external provider UI hidden on next settings load; **External login** rows retained but inactive                                                                            |
| AC-AUTH-014-05 | Guest on **Login**; **Provider subject** not linked; account exists with same normalized **Provider email**      | External sign-in callback succeeds                                                                    | Form-level error `An account already exists for this email. Sign in with your password, then link {Display name} from My account.` on **Login**; no guest linking screen opened |
| AC-AUTH-014-06 | Signed-in user on **My account**; **External provider linking enabled** is **true**; provider not yet linked     | User clicks **Link {Display name}** and completes **Sensitive account actions** step-up (FR-AUTH-013) | OIDC **link mode** attaches provider to signed-in user; message `External sign-in method linked.`; **External account linked** email to **Profile email** (FR-AUTH-007)         |
| AC-AUTH-014-07 | Signed-in **external-only** user with one linked provider on **My account**                                      | User clicks **Unlink** for the only provider without setting a password                               | Action blocked with message `Set a password before removing your only sign-in method.` and link to **Set password**                                                             |
| AC-AUTH-014-08 | Signed-in **external-only** user on **Set password** (linked from **My account**)                                | User completes **Sensitive account actions** step-up and submits valid password fields                | Local password established; **password last changed at** set; other sessions **not** revoked; **Change password** (FR-AUTH-005) and **Forgot password** become available        |
| AC-AUTH-014-09 | Signed-in user on **My account** **External sign-in methods** section                                            | User views linked provider row where **Provider email** differs from **Profile email**                | Inline hint `May differ from your profile email.` shown on the row                                                                                                              |
| AC-AUTH-014-10 | Administrator with **Users.Manage** on **User details** (FR-USR-004); **External providers enabled** is **true** | User clicks **Unlink** for a provider and confirms `Remove {Display name} sign-in from this account?` | **External login** row removed; **External account unlinked** email sent (FR-AUTH-007); same last-sign-in-method rules as self-service unlink apply                             |
