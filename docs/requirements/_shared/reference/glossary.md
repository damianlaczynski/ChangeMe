...
| **Account invitation**             | One invitation send recorded for a user: **sent at**, lifecycle **pending** (active), **revoked** (superseded by **Resend invitation** or **Cancel invitation**), or **accepted** (onboarding complete; kept for history). **Revoked** rows are purged after the configured retention period (default **7** days). See FR-INV-006. |
| **Awaiting invitation acceptance** | The user has a **pending** account invitation and has **not** completed onboarding. They are not yet an active application user (no local password and no completed external onboarding). Sign-in and acceptance: FR-AUTH-010, FR-AUTH-014.                                                                                       |
| **Invitation expired**             | The current invitation **link** is missing, already used, or past its expiry. This is independent of whether a pending invitation still exists in the system. UI may show **`Expired`** while **Resend invitation** remains available (FR-INV-003).                                                                                |
...
| **Passkey enrolled**      | The user has at least one **Passkey credential** (FR-PKY-003). Passkey sign-in is available when **Passkeys authentication enabled** is **true** (FR-PKY-001).                                                                                                                           |
| **Passkey-only account**  | The user has at least one passkey, **no local password**, and **no external login**; allowed only when **Allow passkey-only accounts** is **true** (FR-PKY-001).                                                                                                                          |
...
| **Profile email**  | The **current email** on the ChangeMe account; used for sign-in, display, and all notifications (FR-AUTH-014). Shown as **Email** on **My account** and admin screens.                                                                                                |

## Cross-references

- Invitation flows: `docs/requirements/functional/invitations/`
- Invitation acceptance: FR-AUTH-010
- External sign-in: FR-AUTH-014
- Email verification: FR-AUTH-011
- Self-service email change: FR-AUTH-015
- Passkeys: `docs/requirements/functional/passkeys/`
- User **Status** rules: FR-INV-005
