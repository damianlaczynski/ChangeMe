...
> Template for a **single atomic** functional specification. Copy to `docs/requirements/functional/<domain>/fr-<area>-<nnn>-<slug>.md` and add a row to the change record in `docs/requirements/changes/`.
...
1. Add or update a record in `docs/requirements/changes/`.
2. Run `npm run requirements:validate`.
