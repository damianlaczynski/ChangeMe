...
public sealed class RequestEmailVerificationHandlerTests
{
  ...
  [Fact]
  public async Task Handle_WhenEmailDeliveryFails_ShouldReturnErrorWithoutPersistingToken()
  {
    var cancellationToken = TestContext.Current.CancellationToken;
    await using var context = UseCasesTestDb.Create(
      nameof(Handle_WhenEmailDeliveryFails_ShouldReturnErrorWithoutPersistingToken));
    var authOptions = TestAuthOptions.Create(emailVerificationEnabled: true);
    var passwordHasher = new PasswordHasherAdapter();
    ...
    Assert.Contains(FailingAuthEmailService.DefaultErrorMessage, result.Errors);
    Assert.Empty(context.UserAuthTokens);
  }
}
